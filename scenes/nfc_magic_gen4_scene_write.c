...
bool nfc_magic_gen4_write_worker_callback(NfcMagicWorkerEvent event, void* context) {
    furi_assert(context);
...
static void nfc_magic_gen4_scene_write_setup_view(NfcMagic* nfc_magic) {
...
void nfc_magic_gen4_scene_write_on_enter(void* context) {
    NfcMagic* nfc_magic = context;

    scene_manager_set_scene_state(
        nfc_magic->scene_manager, NfcMagicSceneWrite, NfcMagicSceneWriteStateCardSearch);
    nfc_magic_gen4_scene_write_setup_view(nfc_magic);
...
bool nfc_magic_gen4_scene_write_on_event(void* context, SceneManagerEvent event) {
    NfcMagic* nfc_magic = context;
    bool consumed = false;
...
void nfc_magic_gen4_scene_write_on_exit(void* context) {
    NfcMagic* nfc_magic = context;
