...
void nfc_magic_gen4_scene_success_popup_callback(void* context) {
    NfcMagic* nfc_magic = context;
    view_dispatcher_send_custom_event(nfc_magic->view_dispatcher, NfcMagicCustomEventViewExit);
}

void nfc_magic_gen4_scene_success_on_enter(void* context) {
    NfcMagic* nfc_magic = context;
...
bool nfc_magic_gen4_scene_success_on_event(void* context, SceneManagerEvent event) {
    NfcMagic* nfc_magic = context;
    bool consumed = false;
...
void nfc_magic_gen4_scene_success_on_exit(void* context) {
    NfcMagic* nfc_magic = context;
