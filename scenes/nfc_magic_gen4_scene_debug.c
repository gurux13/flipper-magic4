...
enum {
    NfcMagicSceneDebugStateCardSearch,
    NfcMagicSceneDebugStateCardFound,
};

bool nfc_magic_gen4_debug_worker_callback(NfcMagicWorkerEvent event, void* context) {
    furi_assert(context);
...
static void nfc_magic_gen4_scene_debug_setup_view(NfcMagic* nfc_magic) {
    Popup* popup = nfc_magic->popup;
    popup_reset(popup);
    uint32_t state = scene_manager_get_scene_state(nfc_magic->scene_manager, NfcMagicSceneDebug);

    if(state == NfcMagicSceneDebugStateCardSearch) {
...
void nfc_magic_gen4_scene_debug_on_enter(void* context) {
    NfcMagic* nfc_magic = context;

    scene_manager_set_scene_state(
        nfc_magic->scene_manager, NfcMagicSceneDebug, NfcMagicSceneDebugStateCardSearch);
    nfc_magic_gen4_scene_debug_setup_view(nfc_magic);
    ...
    nfc_magic_gen4_worker_start(
        ...
        nfc_magic_gen4_debug_worker_callback,
        nfc_magic);
    nfc_magic_gen4_blink_start(nfc_magic);
}

bool nfc_magic_gen4_scene_debug_on_event(void* context, SceneManagerEvent event) {
    NfcMagic* nfc_magic = context;
    bool consumed = false;
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == NfcMagicWorkerEventSuccess) {
            ...
            scene_manager_set_scene_state(
                nfc_magic->scene_manager, NfcMagicSceneDebug, NfcMagicSceneDebugStateCardFound);
            nfc_magic_gen4_scene_debug_setup_view(nfc_magic);
            consumed = true;
...
void nfc_magic_gen4_scene_debug_on_exit(void* context) {
    NfcMagic* nfc_magic = context;
