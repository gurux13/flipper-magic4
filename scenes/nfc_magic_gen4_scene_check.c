...
bool nfc_magic_gen4_check_worker_callback(NfcMagicWorkerEvent event, void* context) {
    furi_assert(context);
...
static void nfc_magic_gen4_scene_check_setup_view(NfcMagic* nfc_magic) {
...
void nfc_magic_gen4_scene_check_on_enter(void* context) {
    NfcMagic* nfc_magic = context;

    scene_manager_set_scene_state(
        nfc_magic->scene_manager, NfcMagicSceneCheck, NfcMagicSceneCheckStateCardSearch);
    nfc_magic_gen4_scene_check_setup_view(nfc_magic);
    ...
    nfc_magic_gen4_worker_start(
        ...
        nfc_magic_gen4_check_worker_callback,
        nfc_magic);
    nfc_magic_gen4_blink_start(nfc_magic);
}

bool nfc_magic_gen4_scene_check_on_event(void* context, SceneManagerEvent event) {
    NfcMagic* nfc_magic = context;
    bool consumed = false;
...
void nfc_magic_gen4_scene_check_on_exit(void* context) {
    NfcMagic* nfc_magic = context;
