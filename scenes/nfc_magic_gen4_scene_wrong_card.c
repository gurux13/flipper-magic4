...
void nfc_magic_gen4_scene_wrong_card_widget_callback(
...
void nfc_magic_gen4_scene_wrong_card_on_enter(void* context) {
    NfcMagic* nfc_magic = context;
    ...
    widget_add_button_element(
        widget, GuiButtonTypeLeft, "Retry", nfc_magic_gen4_scene_wrong_card_widget_callback, nfc_magic);

    // Setup and start worker
    view_dispatcher_switch_to_view(nfc_magic->view_dispatcher, NfcMagicViewWidget);
}

bool nfc_magic_gen4_scene_wrong_card_on_event(void* context, SceneManagerEvent event) {
    NfcMagic* nfc_magic = context;
    bool consumed = false;
...
void nfc_magic_gen4_scene_wrong_card_on_exit(void* context) {
    NfcMagic* nfc_magic = context;
