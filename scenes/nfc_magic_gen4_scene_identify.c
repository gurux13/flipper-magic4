...
bool nfc_magic_gen4_identify_worker_callback(NfcMagicWorkerEvent event, void* context) {
    furi_assert(context);
...
static void nfc_magic_gen4_scene_identify_setup_view(NfcMagic* nfc_magic) {
    ...
    if(state == NfcMagicSceneIdentifyStateCardSearch) {
        ...
        popup_set_header(popup, "Identifying\nDon't move...", 52, 32, AlignLeft, AlignCenter);
    }

    view_dispatcher_switch_to_view(nfc_magic->view_dispatcher, NfcMagicViewPopup);
}

void nfc_magic_gen4_scene_identify_on_enter(void* context) {
    NfcMagic* nfc_magic = context;
    ...
    nfc_magic_gen4_worker_start(
        ...
        NULL,
        nfc_magic_gen4_identify_worker_callback,
        nfc_magic);
    nfc_magic_gen4_blink_start(nfc_magic);
}

bool nfc_magic_gen4_scene_identify_on_event(void* context, SceneManagerEvent event) {
    NfcMagic* nfc_magic = context;
    bool consumed = false;
...
void nfc_magic_gen4_scene_identify_on_exit(void* context) {
    NfcMagic* nfc_magic = context;
