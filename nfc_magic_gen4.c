...
bool nfc_magic_gen4_custom_event_callback(void* context, uint32_t event) {
    furi_assert(context);
    NfcMagic* nfc_magic = context;
    return scene_manager_handle_custom_event(nfc_magic->scene_manager, event);
}

bool nfc_magic_gen4_back_event_callback(void* context) {
    furi_assert(context);
    NfcMagic* nfc_magic = context;
    return scene_manager_handle_back_event(nfc_magic->scene_manager);
}

void nfc_magic_gen4_tick_event_callback(void* context) {
    furi_assert(context);
    NfcMagic* nfc_magic = context;
...
NfcMagic* nfc_magic_gen4_alloc() {
    ...
    nfc_magic->worker = nfc_magic_gen4_worker_alloc();
    nfc_magic->view_dispatcher = view_dispatcher_alloc();
    nfc_magic->scene_manager = scene_manager_alloc(&nfc_magic_gen4_scene_handlers, nfc_magic);
    ...
    view_dispatcher_set_custom_event_callback(
        nfc_magic->view_dispatcher, nfc_magic_gen4_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(
        nfc_magic->view_dispatcher, nfc_magic_gen4_back_event_callback);
    view_dispatcher_set_tick_event_callback(
        nfc_magic->view_dispatcher, nfc_magic_gen4_tick_event_callback, 100);
    ...
    nfc_magic->persisted_state = malloc(sizeof(NfcMagic4State));
    LOAD_MAGIC4_SETTINGS(nfc_magic->persisted_state);
    return nfc_magic;
}
...
void nfc_magic_gen4_free(NfcMagic* nfc_magic) {
    ...
    nfc_magic_gen4_worker_free(nfc_magic->worker);
    ...
    free(nfc_magic->persisted_state);
    free(nfc_magic);
}

static const NotificationSequence nfc_magic_gen4_sequence_blink_start_cyan = {
...
static const NotificationSequence nfc_magic_gen4_sequence_blink_stop = {
    &message_blink_stop,
    NULL,
};

void nfc_magic_gen4_blink_start(NfcMagic* nfc_magic) {
    notification_message(nfc_magic->notifications, &nfc_magic_gen4_sequence_blink_start_cyan);
}

void nfc_magic_gen4_blink_stop(NfcMagic* nfc_magic) {
    notification_message(nfc_magic->notifications, &nfc_magic_gen4_sequence_blink_stop);
}

int32_t nfc_magic_gen4_app(void* p) {
    UNUSED(p);
    NfcMagic* nfc_magic = nfc_magic_gen4_alloc();

    scene_manager_next_scene(nfc_magic->scene_manager, NfcMagicSceneStart);

    view_dispatcher_run(nfc_magic->view_dispatcher);

    nfc_magic_gen4_free(nfc_magic);
